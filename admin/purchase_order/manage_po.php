...
<div class="card card-outline card-primary">
    <div class="card-header">
        <h4 class="card-title"><?php echo isset($id) ? "Purchase Order Details - ".$po_code : 'Yeni Satın Alma Siparişi Oluştur' ?></h4>
    </div>
    <div class="card-body">
        <form action="" id="po-form">
            ...
            <div class="container-fluid">
                <div class="row">
                    <div class="col-md-6">
                        <label class="control-label text-info">Satın Alma Sipariş Kodu</label>
                ...
                <hr>
                <fieldset>
                    <legend class="text-info">Ürün Formu</legend>
                    <div class="row justify-content-center align-items-end">
                        ...
                        <div class="col-md-3">
                            <div class="form-group">
                                <label for="item_id" class="control-label">Ürün</label>
                        ...
                        <div class="col-md-3">
                            <div class="form-group">
                                <label for="qty" class="control-label">Miktar</label>
                                <input type="number" step="any" class="form-control rounded-0" id="qty">
                            </div>
                        </div>
                        <div class="col-md-2 text-center">
                            <div class="form-group">
                                <button type="button" class="btn btn-flat btn-sm btn-primary" id="add_to_list">Listeye Ekle</button>
                            </div>
                        </div>
                </fieldset>
                <hr>
                <table class="table table-striped table-bordered" id="list">
                    ...
                    <tfoot>
                        <tr>
                            <th class="text-right py-1 px-2" colspan="5">Ara Toplam</th>
                            <th class="text-right py-1 px-2 sub-total">0</th>
                        </tr>
                        <tr>
                            <th class="text-right py-1 px-2" colspan="5">İndirim <input style="width:40px !important" name="discount_perc" class='' type="number" min="0" max="100" value="<?php echo isset($discount_perc) ? $discount_perc : 0 ?>">%
                        ...
                        <tr>
                            <th class="text-right py-1 px-2" colspan="5">Vergi <input style="width:40px !important" name="tax_perc" class='' type="number" min="0" max="100" value="<?php echo isset($tax_perc) ? $tax_perc : 0 ?>">%
                        ...
                        <tr>
                            <th class="text-right py-1 px-2" colspan="5">Toplam
    ...
    <div class="card-footer py-1 text-center">
        <button class="btn btn-flat btn-primary" type="submit" form="po-form">Kaydet</button>
        <a class="btn btn-flat btn-dark" href="<?php echo base_url.'/admin?page=purchase_order' ?>">İptal Et</a>
    </div>
</div>
...
<script>
    ...
    $(function(){
        $('.select2').select2({
            placeholder:"Lütfen buradan seçiniz",
            width:'resolve',
        })
        $('#item_id').select2({
            placeholder:"Lütfen önce tedarikçiyi seçin",
            width:'resolve',
        })
        ...
        $('#supplier_id').change(function(){
            ...
            if(!!items[supplier_id]){
                ...
                list_item.then(function(){
                    $('#item_id').select2({
                        placeholder:"Lütfen buradan ürün seçin",
                        width:'resolve',
                    })
                })
            }else{
                list_item.then(function(){
                    $('#item_id').select2({
                        placeholder:"Henüz Listelenen Ürün Yok",
                        width:'resolve',
                    })
                })
            }
        ...
        $('#add_to_list').click(function(){
            ...
            if(item == '' || qty == '' || unit == '' ){
                alert_toast('Lütfen bütün alanları doldurun.','warning');
                return false;
            }
            if($('table#list tbody').find('tr[data-id="'+item+'"]').length > 0){
                alert_toast('Ürün listede zaten mevcut.','error');
                return false;
            }
